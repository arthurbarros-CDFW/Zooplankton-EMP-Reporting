...
Pump_tows$gear<-"Pump"
Macro_tows<-unique(select(Macro_clean,Year,Survey,SurveyRep,StationNZ))
Macro_tows$gear<-"Mysid"
...
for(i in 1:length(target_order)){
  ...
  s_max<-d%>%
    group_by(CommonName)%>%
    summarise(max=sum(max(Indices,na.rm=T)))
  s_max<-sum(s_max$max)
  p<-ggplot(d,aes(Year,Indices,fill=CommonName))+
    geom_bar(stat="identity")+
    theme_classic()+
    scale_fill_manual(values=cop_pal,limits=force)+
...
for(i in 1:length(target_order)){
  ...
  d=Zoop_regional_indices%>%
    filter(Order==o)%>%
    group_by(Order,Year,region_of_estuary)%>%
    dplyr::summarise(Indices=mean(Indices))
  p<-ggplot(d,aes(Year,(Indices),color=Order))+
    geom_line(stat="identity",size=1.5)+
    theme_classic()+
    scale_color_manual(values=order_pal,limits=force)+
    ...
    scale_x_continuous(breaks = round(seq(min(1975), max(2020),
                                          by = 5),1))+
    ylab(expression ("CPUE"~(count/m^3)))+
    xlab("")+
