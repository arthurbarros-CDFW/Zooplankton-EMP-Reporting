...
Macro_clean<-readRDS("Data/Macro_clean.rds")
...
######################################
#target adults
target_adults<-c("ACARTELA","ACARTIA","EURYTEM","PDIAPFOR","SINOCAL","OITHDAV","LIMNOSPP","LIMNOSINE","LIMNOTET","OTHCYC","KERATELA","OTHROT","POLYARTH","SYNCH","SYNCHBIC","TRICHO","BOSMINA","DAPHNIA","DIAPHAN","OTHCLADO","H_longirostris","N_kadiakensis","N_mercedis","Amphipod_Total")
...
Macro_targets<-Macro_clean%>%
  filter(Order%in%Macro_orders,Current=="yes")
...
Zoop_winter<-Zoop_seasonal%>%
  filter(seasons=="Winter",Year>=1995,water_year<2021) #also filter out last water year so that we don't have one month of data at end of figures
...
Zoop_regional_indices<-Zoop_regional%>%
  group_by(Year,CommonName,Order,region_of_estuary)%>%
  summarise(Indices=mean(CPUE))
...
test<-unique(dplyr::select(Year_seasonal_regional,SampleDate,StationNZ,water_year,seasons))
